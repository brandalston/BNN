...
def mnist_train_per_class(examples_per_class, examples_skip):
...
def mnist_numpy(dataset, n_instances):
    images = np.zeros((n_instances,28*28), dtype=np.float32)
    labels = np.zeros(n_instances, dtype=int)
    img_id = 0
...
def mnist_train_numpy(n_instances = 60000):
    return mnist_numpy("training", n_instances)


def mnist_test_numpy(n_instances = 10000):
    return mnist_numpy("testing", n_instances)
...
def read(dataset = "training"):
    ...
    if dataset is "training":
        fname_img = 'mnist_data/train-images.idx3-ubyte'
        fname_lbl = 'mnist_data/train-labels.idx1-ubyte'
    elif dataset is "testing":
        fname_img = 'mnist_data/t10k-images.idx3-ubyte'
        fname_lbl = 'mnist_data/t10k-labels.idx1-ubyte'
    else:
