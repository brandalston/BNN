...
for ec in examples_per_class:
    for lr in learning_rate:
        for tfs in tf_seed:
            model_runs.gd_run(["-m", 'GD-binary', "-h", 0, "-s", f's_{0}', "-r", lr, "-c", ec, "-t", time_limit, "-i", tfs, "-f", 'bnn_runs.csv'])
...
for ec in examples_per_class:
    for lr in learning_rate:
        for tfs in tf_seed:
            model_runs.gd_run(["-m", 'GD-ternary', "-h", 0, "-s", f's_{0}', "-r", lr, "-c", ec, "-t", time_limit, "-i", tfs, "-f", 'bnn_runs.csv'])
