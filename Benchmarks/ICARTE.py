...
import time, random, math, os, argparse, warnings
warnings.filterwarnings("ignore")
import numpy as np
import tensorflow.compat.v1 as tf
tf.disable_v2_behavior()
from dataload import mnist_train_per_class, mnist_test_numpy


class StandardNeuralNet:
    def __init__(self, n_input_neurons, n_hidden_neurons, n_hidden_layers, n_output_neurons, lr, tf_seed, zero_weights=True):
        ...
        # Parameters
        self.n_input_units = n_input_neurons
        self.n_hidden_units = n_hidden_neurons
        self.n_hidden_layers = n_hidden_layers
        self.n_output_units = n_output_neurons
        ...
        self._mlp_architecture_ternary() if zero_weights else self._mlp_architecture_binary()
    ...
    def _mlp_architecture_binary(self):
        ...
        # Training algorithm
        self.train_step = tf.train.AdamOptimizer(self.learn_rate).minimize(self.hinge)
    ...
    def train(self, train_data, train_labels, test_data, test_labels, time_out):
        ...
        while (time.time() - start) <= time_out * 60:
            ...
            if (step % 100) == 0:
                train_performance, loss = self.sess.run([self.accuracy, self.hinge],
                                                        feed_dict={self.x: train_data, self.y_: train_labels,
                                                                   self.keep_prob: 1.0})
